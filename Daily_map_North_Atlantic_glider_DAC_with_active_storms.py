...
for i,f in enumerate(zip_files):
    ...
    if 'TRACK' in f:
        ...
        if 'CONE' in f:
            ...
            plt.plot(lon_forec_cone,lat_forec_cone,'.-b',markersize=3)
            ...
            plt.plot(lon_best_track,lat_best_track,'or',markersize=8)
            #plt.text(np.mean(lon_best_track),np.mean(lat_best_track),name.split(' ')[-1],fontsize=14,\
             #        fontweight = 'bold')
...
for i,id in enumerate(gliders):
    ...
    if len(df.index) != 0:
        ...
        ax.plot(np.nanmean(df['longitude (degrees_east)']),\
                ...
                markeredgewidth=2,label=id.split('-')[0])
