...
for l=9:size(id_list,1)
    clear target_salt31 target_salt30 target_saltcop time_mat time_vec depth_vec salt_vec
...
c = colorbar;
colormap('jet')
caxis([floor(min(saltg_vec)) ceil(max(saltg_vec))])
cc_vec = unique(floor([floor(min(saltg_vec)):(max(saltg_vec)-min(saltg_vec))/5:ceil(max(saltg_vec)),ceil(max(saltg_vec))]));
set(c,'ytick',cc_vec)
...
%%
% Figure name
Fig_name = [folder,'Along_track_salt_prof_glider_2models_',inst_name,'_',datestr(timeg(1),'mm-dd-yy'),'-',datestr(timeg(end),'mm-dd-yy')];
wysiwyg
