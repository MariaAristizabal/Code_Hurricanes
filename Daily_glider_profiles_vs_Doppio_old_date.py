...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 18 13:42:21 2019
...
for id in gliders:
    ...
    if len(df.index) != 0 :
        ...
        minlondoppio = np.min(lonrhodoppio[oklatdoppio.min()-2:oklatdoppio.max()+2,oklondoppio.min()-2:oklondoppio.max()+2])
        maxlondoppio = np.max(lonrhodoppio[oklatdoppio.min()-2:oklatdoppio.max()+2,oklondoppio.min()-2:oklondoppio.max()+2])
        minlatdoppio = np.min(latrhodoppio[oklatdoppio.min()-2:oklatdoppio.max()+2,oklondoppio.min()-2:oklondoppio.max()+2])
        maxlatdoppio = np.max(latrhodoppio[oklatdoppio.min()-2:oklatdoppio.max()+2,oklondoppio.min()-2:oklondoppio.max()+2])
        ...
        plt.plot(np.nanmean(target_saltdoppio,axis=1),np.nanmean(target_zdoppio,axis=1),'.-',color='maroon',markersize=12,linewidth=2,\
                 label='Doppio'+' '+str(timedoppio[0].year)+' '+'['+str(timedoppio[0])[5:13]+','+str(timedoppio[-1])[5:13]+']')
        plt.ylabel('Depth (m)',fontsize=20)
