...
u31(length(depth31),length(t31),length(lat_pos)) = nan;

for i=1:length(lat_pos)
    for t=1:length(time31(oktime31))
        disp([i,length(time31(oktime31)),t])
        u31(:,t,i) = squeeze(double(ncread(catalog31_uv,'water_u',[lon_pos(1) lat_pos(i) 1 oktime31(t)],[1 1 inf 1])));
    end
end

save('vel_profile_virgin_island_channel_GOFS31.mat','u31','lon_pos','lat_pos','t31')
