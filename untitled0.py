...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 14:37:04 2019
...
#for id in gliders:
    ...
    file = 'along_track_temp_' + id + '_' + str(tini).split()[0] + '_' + str(tend).split()[0]
    ...
    plt.plot(np.nanmean(saltg_gridded,axis=1),-depthg_gridded,'.-b',\
             label=id[:-14]+' '+str(timeg[0])[0:4]+' '+'['+str(timeg[0])[5:19]+','+str(timeg[-1])[5:19]+']')    
    plt.plot(target_saltGOFS,-depthGOFS,'.-',color='lightcoral')
    plt.plot(np.nanmean(target_saltGOFS,axis=1),-depthGOFS,'.-r',markersize=12,linewidth=2,\
             label='GOFS 3.1'+' '+str(timeGOFS[0].year)+' '+'['+str(timeGOFS[0])[5:13]+','+str(timeGOFS[-1])[5:13]+']')     
    plt.plot(target_saltRTOFS,-depthRTOFS,'.-',color='mediumseagreen')
...
#%%    
    plt.figure()
    plt.plot(lonGOFS,latGOFS)
    plt.axis([np.min(long)-5,np.max(long)+5,np.min(latg)-5,np.max(latg)+5])
    
#%%
meshlonGOFS = np.meshgrid(lonGOFS,latGOFS)
meshlatGOFS = np.meshgrid(latGOFS,lonGOFS)

meshlonRTOFS = np.meshgrid(lonRTOFS,latRTOFS)
meshlatRTOFS = np.meshgrid(latRTOFS,lonRTOFS)
...
ax.plot(meshlonGOFS[0][oklatGOFS.min()-2:oklatGOFS.max()+2,oklonGOFS.min()-2:oklonGOFS.max()+2],\
        meshlatGOFS[0][oklonGOFS.min()-2:oklonGOFS.max()+2,oklatGOFS.min()-2:oklatGOFS.max()+2].T,'.',color='lightcoral')
ax.plot(lonGOFS[oklonGOFS],latGOFS[oklatGOFS],'*r') 
ax.plot(meshlonRTOFS[0][oklatRTOFS.min()-2:oklatRTOFS.max()+2,oklonRTOFS.min()-2:oklonRTOFS.max()+2],\
        meshlatRTOFS[0][oklonRTOFS.min()-2:oklonRTOFS.max()+2,oklatRTOFS.min()-2:oklatRTOFS.max()+2].T,'.',color='mediumseagreen')
ax.plot(lonRTOFS[oklonRTOFS],latRTOFS[oklatRTOFS],'*g') 
ax.plot(meshlonCOP[0][oklatCOP.min()-2:oklatCOP.max()+2,oklonCOP.min()-2:oklonCOP.max()+2]+360,\
        meshlatCOP[0][oklonCOP.min()-2:oklonCOP.max()+2,oklatCOP.min()-2:oklatCOP.max()+2].T,'.',color='plum')
ax.plot(lonCOP[oklonCOP]+360,latCOP[oklatCOP],'*',color='darkorchid') 
...
ax.plot(target_lon,target_lat,'.-b')
#ax.plot(sublon31,sublat31,'.-r')
ax.plot(meshlonRTOFS[0][oklatRTOFS.min()-5:oklatRTOFS.max()+5,oklonRTOFS.min()-5:oklonRTOFS.max()+5],\
        meshlatRTOFS[0][oklonRTOFS.min()-5:oklonRTOFS.max()+5,oklatRTOFS.min()-5:oklatRTOFS.max()+5].T,'*k')
ax.plot(lonRTOFS[oklonRTOFS],latRTOFS[oklatRTOFS],'*g') 
