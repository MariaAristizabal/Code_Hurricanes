...
#prefix_ab = ['rtofs_glo.t00z.n-24.archv','rtofs_glo.t00z.n-18.archv',\
#             'rtofs_glo.t00z.n-12.archv','rtofs_glo.t00z.n-06.archv']

# RTOFS grid file name North Atlantic Region
#gridfile = '/Volumes/aristizabal/ncep_model/HWRF-Hycom-WW3_exp_Michael/hwrf_rtofs_hat10.basin.regional.grid'

# RTOFS depth file name North Atlantic Region
#depthfile = '/Volumes/aristizabal/ncep_model/HWRF-Hycom-WW3_exp_Michael/hwrf_rtofs_hat10.basin.regional.depth'
...
#%%  Reading RTOFS grid North Atlantic region
'''
# Reading lat and lon
lines_grid=[line.rstrip() for line in open(gridfile+'.b')]
#hlon = np.array(readgrids(gridfile,'relax'))

lonRTOFS = np.array(readgrids(gridfile,'plon:',[0]))
latRTOFS = np.array(readgrids(gridfile,'plat:',[0]))
...
#for t in np.arange(2):
    ...
    # Download a/b files
    #ftp.retrbinary('RETR '+file+'.b', open(file+'.b','wb').write)
    #ftp.retrbinary('RETR '+file+'.a', open(file+'.a','wb').write)
...
tRTOFS = np.asarray(mdates.num2date(mdates.date2num(tRTOFS)))
    
#%% Read RTOFS time from a/b files
'''
nz = 41
layers = np.arange(0,nz)
target_tempRTOFS = np.empty((len(prefix_ab),nz,))
target_tempRTOFS[:] = np.nan
target_zRTOFS = np.empty((len(prefix_ab),nz,))
...
for t in np.arange(2):
    #t=0
    file = prefix_ab[t]   
    lines = [line.rstrip() for line in open(file+'.b')]    
    time_stamp = lines[-1].split()[2]
    hycom_days = lines[-1].split()[3]
    tzero=datetime(1901,1,1,0,0)
    timeRT = tzero+timedelta(float(hycom_days))
    tRTOFS.append(timeRT)
    
tRTOFS = np.asarray(tRTOFS)
 '''
...
#%%
    ...
    print(t)
    inFile = prefix_ab[t]
    inFile = 'rtofs_glo.t00z.n-24.archv'
    fileType = 'archive'
    fieldName = 'temp' 
    pntidx=None
    print(inFile)
...
#%%        
    ...
    target_tempRTOFS[t,:] = fld2[oklatRTOFS[t],oklonRTOFS[t],:]
'''
