...
for id in gliders:
    ...
    if len(df.index) != 0 :
        ...
        if len(oktime_amseas) != 0:
            plt.plot(np.nanmean(target_temp_amseas,axis=1),-depthamseas,'.-',color='darkolivegreen',markersize=12,linewidth=2,\
                label='AMSEAS'+' '+str(time_amseas[0].year)+' '+'['+str(time_amseas[0])[5:13]+','+str(time_amseas[-1])[5:13]+']')
        ...
        if len(oklat_amseas) != 0:
            ax.plot(meshlon_amseasg[oklat_amseas.min()-2:oklat_amseas.max()+2,oklon_amseas.min()-2:oklon_amseas.max()+2],\
                meshlat_amseasg[oklon_amseas.min()-2:oklon_amseas.max()+2,oklat_amseas.min()-2:oklat_amseas.max()+2].T,\
        ...
        if len(oktime_amseas) != 0:
            plt.plot(np.nanmean(target_salt_amseas,axis=1),-depthamseas,'.-',color='darkolivegreen',markersize=12,linewidth=2,\
                label='AMSEAS'+' '+str(time_amseas[0].year)+' '+'['+str(time_amseas[0])[5:13]+','+str(time_amseas[-1])[5:13]+']')
        plt.ylabel('Depth (m)',fontsize=20)
