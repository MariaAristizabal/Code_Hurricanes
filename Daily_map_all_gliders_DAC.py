...
col = ['red','sandybrown','gold','m','lightcoral','coral','olive',\
       'crimson''chocolate','hotpink','brown','lightsalmon',\
       'yellow','orchid','lightpink']
...
for i,id in enumerate(gliders):
    if id[0:3] != 'all':
        ...
        ax.plot(df['longitude (degrees_east)'],\
                df['latitude (degrees_north)'],'.-',color=col[i],\
                label=id.split('-')[0],markersize=5) 
...
id_url = 'https://gliders.ioos.us/providers/api/deployment'
