...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 21 10:10:33 2019

@author: aristizabal
"""
#%% User input

mat_file = '/Users/aristizabal/Desktop/MARACOOS_project/Maria_scripts/Figures/Model_glider_comp/ng291_depth_aver_vel.mat'
...
ng291 = sio.loadmat(mat_file)

#%%
uglider = ng291['ug'][:,0]
vglider = ng291['vg'][:,0]
tstamp_glider = ng291['timeg'][:,0]

tstamp_31 =  ng291['time31'][:,0]
oktime31 = ng291['oktime31'][:,0]
depth31 = ng291['depth31'][:,0]

u31 = ng291['target_u31']
v31 = ng291['target_v31']
...
plt.plot(Ug,Vg,'.b',zorder=10,label=ng291['inst_id'][0].split('-')[0])
...
file = folder + 'depth_ave_vel_' + ng291['inst_id'][0].split('-')[0]
...
Q1 = plt.quiver(Xg,Yg,Ug,Vg,scale=0.2,scale_units='inches',color='b',\
                label=ng291['inst_id'][0].split('-')[0],alpha=0.5)
...
file = folder + 'Depth_Average_Velocity_' + ng291['inst_id'][0].split('-')[0]
...
qk = plt.quiverkey(Q1, 0.88, 0.92, 0.1, r'$0.1 \frac{m}{s}$', labelpos='E',
                   coordinates='figure')
#qk = plt.quiverkey(Q2, 0.82, 0.92, 0.1, r'$0.1 \frac{m}{s}$', labelpos='E',
#                   coordinates='figure')
plt.legend()
plt.title('Depth Average Velocity ',size=20)
xfmt = mdates.DateFormatter('%d-%b-%y')
